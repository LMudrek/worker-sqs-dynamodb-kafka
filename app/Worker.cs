...
public sealed record KafkaConversationMessage(string Content, string Role);
    ...
    public async Task PublishAsync(KafkaConversationPayload payload, CancellationToken cancellationToken)
    {
        ...
        if (deliveryResult.Status != PersistenceStatus.Persisted)
        {
            throw new InvalidOperationException(
                $"Kafka message was not persisted. Topic={deliveryResult.Topic}, Partition={deliveryResult.Partition}, Offset={deliveryResult.Offset}");
        }
    }
